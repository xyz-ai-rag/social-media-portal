...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (pathname !== previousPathname.current) {
      debugLog(
        "path",
        `Path changed from "${previousPathname.current}" to "${pathname}"`
      );
  ...
  const isBusinessPage = useCallback(() => {
    ...
    if (/^\/business(es)?($|\/|\/[^\/]+)/.test(pathname)) {
      debugLog(
        "path",
        `isBusinessPage: "${pathname}" matches business pattern`
      );
      return true;
    }
    ...
    if (pathParts.length >= 3) {
      ...
      if (businessSections.includes(pathParts[2])) {
        debugLog(
          "path",
          `isBusinessPage: "${pathname}" matches clientId/businessId/section pattern for section "${pathParts[2]}"`
        );
        return true;
      }
      ...
      if (pathParts.length >= 4) {
        if (pathParts[2] === "topic-analysis") {
          debugLog(
            "path",
            `isBusinessPage: "${pathname}" matches topic-analysis sub-page pattern`
          );
          return true;
        }
  ...
  const isSpecificBusinessPage = useCallback(() => {
    ...
    debugLog(
      "path",
      `isSpecificBusinessPage check for "${pathname}": ${result}`
    );
    return result;
  }, [pathname]);
      ...
      try {
        ...
        if (error) {
          ...
          if (!sessionData.session) {
            debugLog(
              "session",
              "No valid session found during token refresh, logging out"
            );
            cleanupAllIntervals();
            setUser(null);
  ...
  const setupSessionPolling = useCallback(
    (email: string) => {
      ...
      if (pollingActive.current || isResetPasswordFlow()) {
        if (isResetPasswordFlow()) {
          debugLog(
            "session",
            "Skipping session polling due to reset password flow"
          );
  ...
  const shouldRedirectToBusiness = useCallback(
    (clientDetailsData: ClientDetails) => {
      debugLog(
        "redirect",
        `shouldRedirectToBusiness check: path="${pathname}", initialLoad=${isInitialLoad.current}, hasRedirected=${hasRedirected.current}`
      );
      ...
      if (typeof window !== "undefined") {
        ...
        if (lastPath) {
          debugLog(
            "redirect",
            `Found saved path in localStorage: "${lastPath}"`
          );
      ...
      if (
        ...
        debugLog(
          "redirect",
          "REDIRECT TRIGGERED: Redirecting to businesses page on initial load"
        );
      ...
      if (session?.user?.email) {
        ...
        try {
          ...
          const resp = await fetch(
            constructVercelURL(
              `/api/client-details?email=${encodeURIComponent(u.email!)}`
            )
          );
          ...
          if (_event === "SIGNED_IN") {
            debugLog(
              "redirect",
              `SIGNED_IN event processing - inResetFlow=${inResetFlow}, isBusinessPage=${isBusinessPage()}, hasRedirected=${
                hasRedirected.current
              }`
            );
            ...
            if (!inResetFlow && !isBusinessPage() && !hasRedirected.current) {
              if (json?.id && json.businesses?.length) {
                debugLog(
                  "redirect",
                  "REDIRECT TRIGGERED: Auto-redirecting to businesses after sign in"
                );
                hasRedirected.current = true;
              ...
              debugLog(
                "redirect",
                "Already on a business page, skipping redirect"
              );
            }
          } else {
            debugLog(
              "auth",
              `Not a SIGNED_IN event (${_event}), skipping redirect check`
            );
          }
        } catch (err) {
        ...
        if (!inResetFlow) {
          ...
          if (_event === "SIGNED_OUT" && !pathname.startsWith("/auth/")) {
            debugLog(
              "redirect",
              "REDIRECT TRIGGERED: SIGNED_OUT, redirecting to login"
            );
            router.replace("/auth/login");
          }
        } else {
      ...
      if (session?.user?.email) {
        ...
        fetch(
          ...
          .then((d) => {
            ...
            debugLog(
              "redirect",
              `Redirect check: lastPath=${lastPath}, isInitialPageLoad=${isInitialPageLoad}, isBusinessPage=${isBusinessPage()}`
            );

            if (lastPath && isInitialPageLoad) {
              debugLog(
                "redirect",
                `REDIRECT TRIGGERED: Restoring path from localStorage: "${lastPath}"`
              );
              hasRedirected.current = true;
              isInitialLoad.current = false;
            ...
            // Only redirect if it's the initial app load, not a page refresh
              ...
              debugLog(
                "redirect",
                "REDIRECT TRIGGERED: shouldRedirectToBusiness returned true"
              );
              router.replace("/businesses");
            } else {
              debugLog(
                "redirect",
                "Staying on current page - either refresh or already on business page"
              );
          ...
          .catch((e) =>
            console.error("[Auth] initial fetchClientDetails error", e)
          );
      }
      setLoading(false);
    });
  ...
  const login = useCallback(
    async (email: string, password: string) => {
      ...
      try {
        const resp = await fetch(
          constructVercelURL(
            `/api/client-details?email=${encodeURIComponent(email)}`
          )
        );
      ...
      if (isResetPasswordFlow()) {
        debugLog(
          "redirect",
          "In reset password flow, skipping redirect after login"
        );
        return { success: true };
      }
      ...
      if (lastPath) {
        debugLog(
          "redirect",
          `REDIRECT TRIGGERED: Restoring path after login: "${lastPath}"`
        );
        router.replace(lastPath);
        ...
        debugLog(
          "redirect",
          "REDIRECT TRIGGERED: no businesses found, fallback to /auth/login"
        );
        router.replace("/auth/login");
      }
