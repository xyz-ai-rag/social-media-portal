  ...
  try {
    ...
    if (!business_id || !start_date || !end_date) {
      return NextResponse.json(
        {
          error:
            "Missing required parameters: business_id, start_date, end_date",
        },
        { status: 400 }
      );
    }
    ...
    console.log(
      `[Hashtags] Query params: business_id=${business_id}, start_date=${start_date}, end_date=${end_date}`
    );
    console.log(
      `[Hashtags] Parsed dates: startDateTime=${startDateTime.toISOString()}, endDateTime=${endDateTime.toISOString()}`
    );
    ...
    const consolidatedCounts: Record<
      string,
      { count: number; displayTag: string }
    > = {};
    ...
    Object.entries(rawCounts).forEach(([tag, count]) => {
      ...
      if (consolidatedCounts[normalizedTag]) {
        ...
        if (
          (tag.match(/^#+/) || [""])[0].length >
          (consolidatedCounts[normalizedTag].displayTag.match(/^#+/) || [""])[0]
            .length
        ) {
    ...
    const totalCount = sortedTags.reduce(
