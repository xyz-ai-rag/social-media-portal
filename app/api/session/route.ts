...
export async function POST(request: NextRequest) {
  ...
  if (action) {
    console.log(
      `[Session API] ${action} request${email ? " for " + email : ""}`
    );
  }
  ...
  if (action === "check") {
    ...
    try {
      ...
      // Check user still online
      const user = await ClientUsersModel.findOne({
        where: { id: existing.user_id },
        attributes: ["is_active"],
      });

      if (!user?.is_active) {
        // Frontend can get the data of active jump to login page
        return NextResponse.json({ success: true, active: false });
      }
