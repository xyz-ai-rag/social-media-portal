...
const CompetitorPosts: FC<CompetitorPostsProps> = ({ clientId, businessId }) => {
  ...
  useEffect(() => {
    const fetchCompetitors = async () => {
      try {
        ...
        if (!currentBusiness || !currentBusiness.similar_businesses || currentBusiness.similar_businesses.length === 0) {
        ...
        const response = await fetch(constructVercelURL("/api/businesses/getBusinessName/batch"), {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ businessIds: currentBusiness.similar_businesses }),
        });
  ...
  const fetchPostsForPage = useCallback(async (pageNumber: number) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Failed to fetch competitor posts');
      }
