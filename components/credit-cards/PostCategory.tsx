...
interface PostCategoryData {
  postCategoryStats: PostCategoryStat[];
  totalCount: number;
}
...
export default function PostCategory({ businessId }: PostCategoryProps) {
  ...
  useEffect(() => {
    ...
    async function fetchPostCategoryData() {
      setIsLoading(true);
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error("Failed to fetch post category data");
        }
        ...
        if (isCurrent) {
          ...
          const statsWithColors = data.postCategoryStats.map((stat: PostCategoryStat) => ({
            ...stat,
            color: stat.category === 'Commercial post' ? '#8593ED' : '#5A6ACF',
          }));

          setPostCategoryData({
            postCategoryStats: statsWithColors,
            totalCount: data.totalCount,
          });
        }
      } catch (error) {
        if (isCurrent) {
          console.error("Error fetching post category data:", error);
        }
      } finally {
        if (isCurrent) {
          setIsLoading(false);
  ...
  useEffect(() => {
    ...
    const seriesData = postCategoryData.postCategoryStats.map((item) => ({
      name: item.category,
      value: item.count,
    ...
    const option = {
      ...
      graphic: {
        ...
        style: {
          text:
            postCategoryData.totalCount > 0
              ? `${postCategoryData.totalCount.toLocaleString()}\nPosts`
              : "No Data",
          textAlign: "center",
          color: "#333",
          fontSize: 16,
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md w-full min-h-[400px] flex flex-col">
      <div className="flex-1 flex flex-col">
        ...
        ) : postCategoryData.postCategoryStats.length === 0 ? (
          <div className="h-64 flex items-center justify-center w-full">
            <p className="text-gray-500">No post category data available</p>
          </div>
        ) : (
          <>
            <div className="mb-2">
              <h2 className="text-base font-medium text-gray-800">Post Category</h2>
            </div>
            ...
            <div className="flex flex-wrap justify-center gap-10 mt-4 w-full">
              {postCategoryData.postCategoryStats.map((item, index) => (
