...
import { FiChevronLeft, FiChevronRight, FiSettings, FiLogOut, FiGrid, FiList, FiUsers, FiAlertCircle } from 'react-icons/fi';
...
const MenuItem: React.FC<MenuItemProps> = ({ 
  ...
  if (disabled) {
    return (
      <div 
        className={`flex items-center p-2 rounded-md cursor-not-allowed ${collapsed ? 'justify-center' : ''} text-gray-400`}
        onClick={onClick}
      >
...
export default function Sidebar() {
  ...
  const isActive = (path: string): boolean => {
    if (path.includes('[clientId]') && path.includes('[businessId]') && currentClientId && currentBusinessId) {
  ...
  const currentBusiness = hasSelectedBusiness
    ? clientDetails?.businesses?.find(biz => biz.business_id === currentBusinessId)
    : null;

  return (
    <aside className={`bg-gray-50 border-r border-gray-200 fixed top-0 bottom-0 left-0 flex flex-col ${collapsed ? 'w-20 overflow-visible' : 'w-64'} transition-all duration-300 z-10`}>
      {/* Top: Client Info */}
        ...
        <div className="flex items-center space-x-3">
          ...
          {!collapsed && (
            <div className="overflow-hidden">
              <div className="text-sm font-bold text-blue-700 truncate">{clientDetails?.client_name || 'Client Name'}</div>
              {currentBusiness && (
                <div className="text-xs text-gray-500 truncate">
                  Viewing: <span className="font-medium">{currentBusiness.business_name}</span>
                </div>
              )}
            </div>
          )}
      ...
      <div className={`flex-1 ${collapsed ? '' : 'overflow-y-auto overflow-x-hidden'}`}>
        <div className="p-4">
          {!collapsed && (
          ...
          <nav className="space-y-2">
            <MenuItem 
              href={hasSelectedBusiness ? `/${currentClientId}/${currentBusinessId}/dashboard` : '#'}
              icon={<FiGrid />}
              label="Dashboard"
            ...
            <MenuItem 
              href={hasSelectedBusiness ? `/${currentClientId}/${currentBusinessId}/posts` : '#'}
              icon={<FiList />}
              label="All Posts"
            ...
            <MenuItem 
              href={hasSelectedBusiness ? `/${currentClientId}/${currentBusinessId}/competitors` : '#'}
              icon={<FiUsers />}
              label="Competitors"
      ...
      <div className="border-t border-gray-200 p-4">
        ...
        <nav className="space-y-2">
          ...
          <div 
            onClick={logout} 
            className={`flex items-center cursor-pointer p-2 rounded-md ${collapsed ? 'justify-center' : ''} text-gray-700 hover:bg-blue-50`}
          >
            <span className="flex items-center relative group">
              <span className="inline-flex items-center justify-center w-6 h-6">
                <FiLogOut />
      ...
      {hasSelectedBusiness && !collapsed && (
        <div className="border-t border-gray-200 p-4">
          <Link href="/businesses" 
            className="flex items-center justify-between text-blue-600 hover:bg-blue-50 p-2 rounded-md text-sm font-medium"
          >
