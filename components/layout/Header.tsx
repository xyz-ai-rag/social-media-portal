...
export default function Header() {
  ...
  const currentBusiness = clientDetails?.businesses?.find(biz => biz.business_id === currentBusinessId);
  ...
  const hasBusinesses = clientDetails && clientDetails.businesses && clientDetails.businesses.length > 0;
      ...
      <div className="flex items-center space-x-4">
        ...
        {showBusinessSelector && hasBusinesses && (
          <div className="relative" ref={dropdownRef}>
            ...
            >
              <span className="text-gray-800 font-medium text-sm mr-1.5">{currentBusinessName}</span>
              <FiChevronDown className={`text-gray-500 transform transition-transform duration-200 ${isDropdownOpen ? 'rotate-180' : ''}`} />
            </div>
